		...
		if (simulate_network_to_test_trained) {
			...
			for (int stimulus_index = 0; stimulus_index < input_neurons->total_number_of_input_stimuli; stimulus_index++){
			    ...
			    // Setting output name
			 	...
			 	//why do we not reset the model here? what does reset model actually do? Presumably reset spike_monitor is what forces it to separate out the spikes between stimuli. 
			 	spike_monitor->reset_state();
		    }
